...
defaultBehaviours = {
    ...
    idle : function(timeDiff) {
        ...
        if (this.timeToChill < 0) {
            this.timeToChill = ~~(Math.random() * 2 + 1);
            //1-3s
...
charStates = {
    idle : {
        live : defaultBehaviours.idle
    },
    moveTo : {
        live : defaultBehaviours.moveTo
    },
    engage : {
        live : defaultBehaviours.engage
    },
    flee : {
        live : defaultBehaviours.flee
    },
}
...
Character = function(scene, params, initStats) {
    ...
    if (initStats === undefined)
        var initStats = {};
    if (initStats.hp === undefined)
        initStats.hp = 0.5 + Math.random() * 0.5;
    if (initStats.dex === undefined)
        initStats.dex = 0.5 + Math.random() * 0.5;
    if (initStats.str === undefined)
        initStats.str = 0.5 + Math.random() * 0.5;
    if (initStats.spd === undefined)
        initStats.spd = 0.5 + Math.random() * 0.5;

    this.charState = params.charState !== undefined ? charStates[params.charState] : undefined;
    this.lastTime = 0;
    this.timeToChill = ~~(Math.random() * 10);
    //the beat of life, with these, the creature shall live.
    ...
    this._vphyBody.character = this;
    //bind the character to the physics body...(for identificatoin on contact)
    /*this.connectedChars = [];
...
Character.prototype.pickUp = function(someLoot) {
    if (teamLoot[this.team][someLoot.code] === undefined)
        teamLoot[this.team][someLoot.code] = {
            q : 0,
            type : 0
        };
...
Character.prototype.getStat = function(stat) {
    if (this.baseClass.stats[stat] === undefined)
        return undefined;
    return calcStat(this.statMods[stat], this.baseClass.stats[stat]);
    //.M-this.baseClass.stats[stat].m)+this.baseClass.stats[stat].m;
}
...
Character.prototype.getWStat = function(stat) {
    ...
    return calcStat(this.statMods[this.weapon][stat], this.weapon.data[stat]);
    //.M-this.weapon[stat].m)+this.weapon[stat].m;
}
...
Character.prototype.disArm = function() {
    ...
    if (!(this.weapon === undefined || this.weapon.code === this.baseClass.defWep.charCodeAt(0))) {
        if (this.weapon.code)
            teamLoot[this.team][this.weapon.code].q += 1;
    }
    //clear
...
Character.prototype.setWeapon = function(WepChar) {
    this.weapon = {};
    //make a new container for this character to store weapon data
    ...
    if (this.aimPoint === undefined)
        this.aimPoint = new THREE.Vector3(Math.random() * 200, Math.random() * 200, Math.random() * 200).addSelf(this.getPos());
...
Character.prototype.doAttack = function(timeDiff) {
    ...
    if (this.weapon === undefined) {
        ...
        this.setWeapon(this.baseClass.defWep);
        //arm
        return;
        //but don't shoot yet...
    }
    ...
    var fa = this.getWStat('fa');
    //check for firing accuracy... represents a projectile weapon.
...
Character.prototype.takeHit = function(hitter, force, hp) {
    ...
    if (this.HP < 0) {
        ...
        this.dead = true;
        //dead characters still hang around.
        this.dropLoot();
        this.color[1] = 0;
...
Character.prototype.live = function(timeDiff) {
    ...
    if (!this.dead) {
        this.updateEffectiveness();
        if (this.charState)
            this.charState.live.call(this, timeDiff);
        ...
        if (this.weaponSprite !== undefined && this.target !== undefined) {
            ...
            if (this === player)
                testSprite.position.copy(this.aimPoint);
